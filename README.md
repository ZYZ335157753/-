...
一个可在 VS Code 中运行的 Python/Flask 小程序：顾客可浏览带图茶单、加入底部购物袋、填写取餐称呼和手机尾号四位、以**模拟支付**完成下单并获得三位取餐号；管理员可登录后台查看订单并更新制作状态。视觉素材为本项目生成并随仓库交付。
...
创建订单示例（服务端生成 `pickup_no`，顾客凭此号码取餐）：
