...
def test_menu_and_order(tmp_path: Path):
    ...
    assert len(menu) >= 6
    order = client.post("/api/v1/orders", json={"customer_name": "测试顾客", "phone_last4": "1234", "items": [{"product_id": menu[0]["id"], "quantity": 2}]} )
    ...
    assert found.get_json()["order"]["pickup_no"] == "001"


def test_rejects_unknown_product(tmp_path: Path):
    app = create_app({"TESTING": True, "DATABASE": tmp_path / "test.db", "SECRET_KEY": "test"})
    response = app.test_client().post("/api/v1/orders", json={"customer_name": "测试", "phone_last4": "1234", "items": [{"product_id": "missing", "quantity": 1}]})
